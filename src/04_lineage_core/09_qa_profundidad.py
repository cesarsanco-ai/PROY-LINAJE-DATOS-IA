#antes: 5_8_test_dependencias.py

# actual: src/04_lineage_core/09_qa_profundidad.py
import pandas as pd
import json
import os
import sys
...
def construir_arbol_trazabilidad(tabla_objetivo, metadata_sp, mapeo_nombre_a_id_sp, mapeo_id_a_nombre_tabla, mapeo_nombre_a_id_tabla, max_profundidad=10):
    ...
    # Cargar dependencias del CSV (Raw)
    try:
...
def mostrar_arbol_trazabilidad(arbol):
    ...
    print(f"\n🎯 RESUMEN DEL ÁRBOL DE TRAZABILIDAD (QA TEST)")
    print("=" * 50)
...
def main():
    ...
    Función principal
    """
    print("🔍 SISTEMA DE QA - TEST DE PROFUNDIDAD (V7)")
    print("=" * 40)
    print("💡 Este script valida que el grafo tenga continuidad y no haya ciclos infinitos.")
    print("   Úsalo para probar una tabla crítica antes de generar el maestro completo.\n")
    ...
    print(f"\n🚀 Iniciando QA completo de: {tabla}")
    print("⏳ Analizando...\n")
